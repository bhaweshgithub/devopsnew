...
def createAccount(accountemail,accountname,username,securityemail):
    ...
    #generate ouname
    oulist = accountname.split("_")
    ouname = ['Customers']
    if len(oulist) > 3:
        for oun in oulist[:3]:
            ouname.append(oun)
        s = slice(3,len(oulist))
        accountnamepart = str.join("_",oulist[s])
    elif (len(oulist)<=3):
        s = slice(0,len(oulist)-1)
        for oun in oulist[s]:
            ouname.append(oun)
        accountnamepart = oulist[len(oulist)-1]


    inputjson = { 'accountemail' : accountemail , 'accountname' : accountnamepart, 'username' : username, 'securityemail' : securityemail, 'ouname' : ",".join(ouname), 'enabledregions' : 'eu-central-1' }
    response = stepfunctions.start_execution(
        stateMachineArn='arn:aws:states:eu-central-1:275662325630:stateMachine:createNewAccount-PROD',
